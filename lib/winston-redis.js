...
var Redis = exports.Redis = function (options) {
  ...
  this.globalMeta  = options.globalMeta || {};
...
Redis.prototype.log = function (level, msg, meta, callback) {
  ...
  this.redis.llen(container, function (err, len) {
    ...
    // Global property is added only if it is not already available in the meta
    for (var globalKey in self.globalMeta) {
      var globalValue = self.globalMeta[globalKey];

      if (globalValue && (meta[globalKey] == null || typeof meta[globalKey] != 'undefined')) {
        meta[globalKey] = globalValue;
      }
    }
